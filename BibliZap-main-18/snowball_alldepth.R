...
pmid_list<- c('10.1097/MOU.0b013e3283381793', '10.1016/j.jelectrocard.2014.11.005', '10.1093/aje/kwab187', '10.1038/modpathol.2013.68', "10.1002/aur.1471", "10.1055/s-0043-1771454", "10.1016/j.brat.2022.104170", "10.1136/ard.61.4.298", "10.1007/s10900-017-0316-9", "10.1002/jrsm.1635")
...
columns = c("authors","year_published", "journal", "title", "abstract", "scholarly_citations_count", "Freq","doi") 
df = data.frame(matrix(nrow = 0, ncol = length(columns))) 
colnames(df) = columns
for (doi in pmid_list){
  bb<- snowball_bibliography(doi)
  bb$year_published<-bb$year_published-bb$year_published[1]
  bb$Freq<-bb$Freq/bb$Freq[1]
  df<-rbind(df, bb)
}
columns = c("doi", "depth", "results") 
df = data.frame(matrix(nrow = 0, ncol = length(columns))) 
colnames(df) = columns
for (doi in pmid_list){
  df<-rbind(c(doi, 1, snowball_bibliography(doi, depth=1)), df)
  df<-rbind(c(doi, 2, snowball_bibliography(doi)), df)
  df<-rbind(c(doi, 3, snowball_bibliography(doi, depth=3)), df)
}
